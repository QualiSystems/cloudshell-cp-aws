...
class SetAppSecurityGroupsOperation:
    ...
    def set_apps_security_groups(
        self, reservation, app_security_group_models, ec2_session, logger
    ):
        """# noqa
        ...
        # we identify the security groups that need to be created or updated, by
        # finding the interfaces
        # interfaces are identified by comparing the subnet id of the interface,
        # with the requested subnet
        # however, when working with "Default Subnet" mode, which is when no subnet
        # was specified in sandbox
        # we instead provide the server will send a subnet id in the form of the
        # vpc CIDR block.
        ...
        for app_security_group_model in app_security_group_models:
            try:
                ...
                security_group_configurations = (
                    app_security_group_model.security_group_configurations
                )

                logger.info(
                    f"Setting custom app security rules for "
                    f"{app_security_group_model.deployed_app.name}."
                )
                ...
                for security_group_configuration in security_group_configurations:
                    ...
                    network_interfaces = filter(
                        lambda x: x.subnet_id == subnet_id
                        or x.vpc.cidr_block == subnet_id,
                        instance.network_interfaces,
                    )

                    for network_interface in network_interfaces:
                        custom_security_group = self.security_group_service.get_or_create_custom_security_group(  # noqa
                        ...
                        self.security_group_service.remove_all_inbound_rules(
                ...
                logger.error(
                    "Setting custom app security rules failed for '{}' with "
                    "error '{}'.".format(
