...
class PortGroupAttributeParser:
    ...
    def parse_port_group_attribute(ports_attribute):
        ...
        if ports_attribute:
            ...
            port_data_array = [
                PortGroupAttributeParser._single_port_parse(port.strip())
    ...
    def _single_port_parse(ports_attribute):
        ...
        from_to_protocol_match = PortGroupAttributeParser.PORT_DATA_MATCH.search(
            ports_attribute
        )
        ...
        raise ValueError(f"The value '{ports_attribute}' is not a valid ports rule")
