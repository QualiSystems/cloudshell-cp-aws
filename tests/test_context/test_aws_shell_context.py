...
from cloudshell.cp.aws.domain.context.aws_shell import (
    AwsShellContext,
    AwsShellContextModel,
)
...
class AwsAwsShellContext(TestCase):
    def test_aws_shell_context_proper_initialized(self):
        ...
        with patch(
            "cloudshell.cp.aws.domain.context.aws_shell.LoggingSessionContext"
        ) as logger:
            with mock.patch(
                "cloudshell.cp.aws.domain.context.aws_shell." "ErrorHandlingContext"
            ):
                with mock.patch(
                    "cloudshell.cp.aws.domain.context.aws_shell."
                    "CloudShellSessionContext"
                ) as cloudshell_session:
                    with mock.patch(
                        "cloudshell.cp.aws.domain.context.aws_shell."
                        "AwsResourceModelContext"
                    ) as aws_ec2_resource_model:
                        with mock.patch(
                            "cloudshell.cp.aws.domain.context.aws_shell."
                            "AwsApiSessionContext"
                        ) as aws_api:
                            expected_context = AwsShellContextModel(
                                logger=logger().__enter__(),
                                cloudshell_session=cloudshell_session().__enter__(),
                                aws_ec2_resource_model=aws_ec2_resource_model().__enter__(),  # noqa
